...
const TestResultsPanel = ({ onCheckClick }) => {
  ...
  // Get the configured API server URL and settings
  const apiServerUrl = editorConfig?.tests?.dataContractCliApiServerUrl || 'https://api.datacontract.com';
  const showSettingsButton = editorConfig?.tests?.showSettings !== false; // Default to true
  const customHelpText = editorConfig?.tests?.helpText;
  ...
  const renderHelp = () => {
    // If custom help text is provided, render it
    if (customHelpText) {
      return (
        <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
          <div className="flex items-start justify-between mb-2">
            <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-200">{t('testResults.help.title')}</h4>
            <button
              onClick={() => setShowHelp(false)}
              className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200"
              title={t('testResults.help.hide')}
            >
          ...
          <div
            ...
            dangerouslySetInnerHTML={{ __html: customHelpText }}
          />
        </div>
      );
    }

    // Default help content for CLI instructions
    return (
      <div className="mt-4 p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg">
        <div className="flex items-start justify-between mb-2">
          <h4 className="text-sm font-semibold text-blue-900 dark:text-blue-200">{t('testResults.help.cliTitle')}</h4>
          <button
            onClick={() => setShowHelp(false)}
            className="text-blue-600 dark:text-blue-400 hover:text-blue-800 dark:hover:text-blue-200"
            title={t('testResults.help.hide')}
        ...
        <div className="text-sm text-blue-800 dark:text-blue-300 space-y-2">
          ...
          <p>{t('testResults.help.apiRunsAt')} <span className="font-mono">{apiServerUrl}</span>.</p>
          <p>
      ...
      return (
        <div className="space-y-3">
          <div className="text-sm text-red-700 dark:text-red-300">
            <p className="font-semibold">{t('testResults.errorLabel')}</p>
        ...
        return (
          <div className="space-y-3">
            ...
            {data.logs && data.logs.length > 0 && (
              <div className="mt-4">
                <h4 className="text-sm font-semibold mb-2 text-gray-700 dark:text-gray-300">{t('testResults.logs')}</h4>
      ...
      <div className="h-full bg-gray-50 dark:bg-gray-900 flex flex-col overflow-hidden">
            ...
            <span className="inline-flex w-full">
              <div className="-mr-px grid grid-cols-1 flex-1">
                <select
                  ...
                  aria-label={t('testResults.selectServer')}
        ...
        <div className="flex-1 flex items-center justify-center text-gray-500 dark:text-gray-400">
          <div className="text-center">
            ...
            <h3 className="mt-2 text-sm font-medium">{t('testResults.empty.title')}</h3>
            <p className="mt-1 text-sm">{t('testResults.empty.subtitle')}</p>
          </div>
        </div>
        ...
        <div className="flex gap-2 items-center">
          ...
          {currentResult && (
            <button
              ...
              title={t('testResults.clearTitle')}
            >
              {t('testResults.clear')}
            </button>
          )}
              ...
              <div className="flex items-center gap-2">
                ...
                <div className="flex flex-col">
                  <span className={`text-sm font-semibold ${
                    ...
                    {currentResult.success ? t('testResults.passed') : t('testResults.failed')}
                  </span>
                  {currentResult.data && currentResult.data.server && (
                    <span className="text-xs text-gray-600 dark:text-gray-400">
                      {t('testResults.serverLabel', { server: currentResult.data.server })}
                    </span>
                  )}
                </div>
              </div>
